...
for rows in mat_reader:
	if rows[1] == 'invisible':
		invisible_materials.append(int(rows[0]))
	else: mat_color[int(rows[0])] = f'  {rows[1]}  {rows[2]}  {rows[3]}  {rows[4]}  '
...
for i,v in enumerate(line_mat):
	if not v:
		...
		elif invisible_materials.count(v) > 0:	# check wether material should be invisible
			out_lines.append(in_lines[i])
			cell_number = re.search(r'!\s+\d+', in_lines[i]).group().split(' ')[-1]
			invisible_cells.append(f'-{cell_number}')
			change = f'cell {cell_number} has invisible material {v}'
		else:
			out_lines.append(in_lines[i])
